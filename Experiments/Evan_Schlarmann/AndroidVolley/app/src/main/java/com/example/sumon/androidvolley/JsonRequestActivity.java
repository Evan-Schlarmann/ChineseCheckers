...
import com.example.sumon.androidvolley.models.userModel;
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Method.GET,
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            user = new userModel(response.getString("name"), response.getString("email"));
                        }
                        catch (JSONException e) {
                            e.printStackTrace();
                        }
                        msgResponse.setText(user.printUser());
                        hideProgressDialog();
                    }
                    ...
                    public void onResponse(JSONArray response) {
                        ...
                        try {
                            for(int i = 0; i < response.length(); i++){
                                ...
                                users.add(new userModel(tempUser.getString("name"), tempUser.getString("email")));
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                        ...
                        for(userModel u : users){
                            userDetails += u.printUser() + "\n";
                        }
                        msgResponse.setText(userDetails);
                        users.clear();
                        hideProgressDialog();
                    }
                }, new Response.ErrorListener() {
